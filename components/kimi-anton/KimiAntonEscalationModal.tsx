...
interface KimiAntonEscalationModalProps {
  isOpen: boolean;
  onClose: () => void;
...
export default function KimiAntonEscalationModal({
  isOpen,
  onClose,
  trigger,
  severity,
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await fetch('/api/kimi-anton/escalate', {
        method: 'POST',
        ...
        {submitted ? (
          <div className="text-center py-4">
            ...
            <p className="text-foreground-secondary text-sm mb-6">
              Anton (and JHawk if needed) will be notified. The escalation is now pending review.
            </p>
            <button
              onClick={onClose}
